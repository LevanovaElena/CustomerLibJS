      ...
      allProperty.forEach((item) => {
        switch (item) {
          ...
          case "_lastName":
            ...
            break;
          case "_addressesList":
            if (!customer._addressesList)
              result.push({
                propertyName: item,
                message: "AddressesList is required.",
              });
            else if (customer._addressesList.length <= 0)
              result.push({
                propertyName: item,
                message: "AddressesList must be at least one address.",
              });
            break;
          case "_phoneNumber":
            if (customer._phoneNumber) {
              if (
                customer._phoneNumber
                  ...
                  .split(" ")
                  .join("")
                  .match(new RegExp(/^\+[1-9]\d{10,14}$/)) === null
              )
                result.push({
                  propertyName: item,
          ...
          case "_notes":
            ...
            else if (customer._notes.length <= 0)
              result.push({
                propertyName: item,
                message: "Notes must be at least one note.",
              });
            break;
          case "_totalPurchasesAmount":
            if (customer._totalPurchasesAmount) {
              if (!Number.parseFloat(customer._totalPurchasesAmount))
                result.push({
                  propertyName: item,
          ...
          case "_lastPurchaseDate":
            if (customer._lastPurchaseDate) {
              if (!Date.parse(customer._lastPurchaseDate))
                result.push({
                  propertyName: item,
