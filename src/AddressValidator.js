    ...
    ValidatorAddress(address) {
        ...
        if(allProperty.length>0)
        {
            allProperty.forEach(item=>{
                switch (item) {
                    case "_addressLine":
                        if(!address._addressLine)result.push({propertyName:item,message:"AddressLine is required."});
                        else if(address._addressLine.length>100)result.push({propertyName:item,message: "AddressLine should maximum 100 lenght."});
                        break;
                    case "_addressLine2":
                        if(address._addressLine2)
                            if(address._addressLine2.length>100)result.push({propertyName:item,message: "AddressLine2 should maximum 100 lenght."});
                        break;
                    case "_typeAddress":
                        if(!address._typeAddress)result.push({propertyName:item,message: "TypeAddress is required."});
                        else if([1,2].indexOf(address._typeAddress)===-1)result.push({propertyName:item,message: "TypeAddress should be 1 or 2."});
                        break;
                    case "_city":
                        if(!address._city)result.push({propertyName:item,message: "City is required."});
                        else if(address._city.length>50)result.push({propertyName:item,message: "City should maximum 50 lenght."});
                        break;
                    case "_postalCode":
